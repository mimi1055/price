...
export function monthlySpend(records, year, month) {
    const start = new Date(year, month, 1);
    const end = new Date(year, month + 1, 1);
    let total = 0;
    for (const r of records) {
        const date = new Date(r.timestamp);
        if (money(r.cost) !== null && date >= start && date < end) total += r.cost;
    }
    return total;
}

export function pageOf(records, requestedPage, pageSize = 10) {
    const pages = Math.max(1, Math.ceil(records.length / pageSize));
    const page = Math.min(Math.max(0, requestedPage), pages - 1);
    return { rows: records.slice(page * pageSize, (page + 1) * pageSize), page, pages };
}
