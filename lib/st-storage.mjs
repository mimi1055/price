...
export class STFileLedger {
    ...
    update(records, { preserveExisting = false } = {}) {
        ...
        const next = this.queue.catch(() => {}).then(async () => {
            ...
            for (const r of changes) {
                if (preserveExisting && merged.has(r.id)) { skipped++; continue; }
                if (!merged.has(r.id)) added++;
                merged.set(r.id, { ...merged.get(r.id), ...r });
            }
            if (preserveExisting && !added) return { added, skipped };
            ...
            return preserveExisting ? { added, skipped } : data.records;
        });
        this.queue = next;
        return next;
    }
