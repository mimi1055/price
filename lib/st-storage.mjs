...
export const FILE_URL = `/files/${FILE_NAME}`;
...
export class STFileLedger {
    ...
    async read() {
        ...
        if (res.status === 404) {
            ...
            const verified = await this.post('/api/files/verify', { urls: [`files/${FILE_NAME}`] });
            if (verified[`files/${FILE_NAME}`] !== false) throw new Error('File unavailable');
            return [];
        }
        if (!res.ok) throw new Error(`ST HTTP ${res.status}`);
