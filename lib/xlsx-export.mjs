...
export function exportLedgerXlsx(records, language = 'zh-TW') {
    ...
    const labels = zh ? {
        ...
        time: '時間', provider: '服務商', character: '角色', chat: '聊天室', kind: '生成類型', status: '狀態', input: '輸入 tokens', output: '輸出 tokens', source: '費用來源', id: '紀錄 ID', note: '備註',
    } : {
        ...
        time: 'Time', provider: 'Provider', character: 'Character', chat: 'Chat', kind: 'Generation type', status: 'Status', input: 'Input tokens', output: 'Output tokens', source: 'Cost source', id: 'Record ID', note: 'Note',
    };
    ...
    const detailRows = [...valid].sort((a, b) => b.timestamp.localeCompare(a.timestamp)).map(r => [r.timestamp, r.model || '', r.cost, r.provider || 'openrouter',
        r.character || '', r.chat_name || '', r.kind === 'connectionTest' ? (zh ? '連線測試' : 'Connection test') : r.kind || '',
        r.status || '', r.input_tokens, r.output_tokens, r.cost_source || '', r.id || '',
        r.kind === 'connectionTest' ? (r.status === 'failed' && r.cost == null
    ...
    const sheets = [
        ...
        [labels.details, sheet([labels.time, labels.model, labels.cost, labels.provider, labels.character, labels.chat, labels.kind, labels.status,
            labels.input, labels.output, labels.source, labels.id, labels.note], detailRows, [23, 42, 20, 17, 22, 28, 20, 24, 18, 18, 22, 39, 48],
        { 0: 'date', 2: 'money', 8: 'number', 9: 'number' })],
    ];
    ...
    const files = [
        ...
        ['xl/styles.xml', `${declaration}<styleSheet xmlns="${main}"><numFmts count="2"><numFmt numFmtId="164" formatCode="&quot;US$&quot;#,##0.00000"/><numFmt numFmtId="165" formatCode="yyyy-mm-dd hh:mm:ss"/></numFmts><fonts count="2"><font><sz val="11"/><name val="Aptos"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="11"/><name val="Aptos"/></font></fonts><fills count="2"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF344334"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="4"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="1" borderId="0" xfId="0" applyFont="1" applyFill="1"/><xf numFmtId="164" fontId="0" fillId="0" borderId="0" xfId="0" applyNumberFormat="1"/><xf numFmtId="165" fontId="0" fillId="0" borderId="0" xfId="0" applyNumberFormat="1"/></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`],
